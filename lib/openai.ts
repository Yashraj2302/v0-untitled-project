...
// Flag to track if we've hit the quota limit
let quotaExceeded = false

export async function getAiStrategyAdvice(
  ...
  // If we've already hit the quota limit, skip the API call entirely
  if (quotaExceeded) {
    console.log("Quota previously exceeded, using mock response directly")
    const mockResponse = await getMockStrategyAdvice(marketCondition)
    return {
      text: mockResponse,
      error:
  ...
  if (!isOpenAIConfigured) {
    return {
      text: await getMockStrategyAdvice(marketCondition),
      error:
        "OpenAI API key is not configured. Please add the OPENAI_API_KEY environment variable to enable AI-powered strategy advice.",
      usingMock: true,
    }
  }
  ...
  try {
    ...
    return { text, error: null, usingMock: false }
  } catch (error) {
    ...
    if (
      ...
      // Set the flag to avoid future API calls
      quotaExceeded = true
    ...
    if (error.message && error.message.includes("API key")) {
      ...
      return {
        text: mockResponse,
        error: "OpenAI API key is missing or invalid. Please add a valid OPENAI_API_KEY to your environment variables.",
        usingMock: true,
      }
    }
    ...
    return {
      text: mockResponse,
      error: "Unable to generate strategy advice at this time. Please try again later.",
      usingMock: true,
    }
  }
}

export async function analyzeBacktestResults(results: string): Promise<{
  text: string
  error: string | null
  usingMock: boolean
}> {
  ...
  if (quotaExceeded) {
    return {
      text: "Backtest analysis is not available due to API quota limitations.",
      error:
        "Your OpenAI API key has reached its quota limit. Please check your billing details or use a different API key.",
      usingMock: true,
    }
  ...
  if (!isOpenAIConfigured) {
    return {
      text: "Backtest analysis is not available without an OpenAI API key.",
      error:
        "OpenAI API key is not configured. Please add the OPENAI_API_KEY environment variable to enable AI-powered analysis.",
      usingMock: true,
    }
  }
  ...
  try {
    ...
    if (error.message && error.message.includes("API key")) {
      return {
        text: "Backtest analysis is not available due to API key issues.",
        error: "OpenAI API key is missing or invalid. Please add a valid OPENAI_API_KEY to your environment variables.",
        usingMock: true,
      }
    }
    ...
    // General error fallback
    return {
      text: "Backtest analysis is not available at this time.",
      error: "Unable to analyze backtest results at this time. Please try again later.",
      usingMock: true,
    }
  }
}
